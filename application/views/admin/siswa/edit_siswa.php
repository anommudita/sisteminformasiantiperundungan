                ...
                <div class="col-lg-6 col-5 text-right">
                    <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
                        <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
                            <li class="breadcrumb-item"><a href="<?php echo site_url('admin'); ?>"><i class="fas fa-users"></i></a></li>
                            <li class="breadcrumb-item"><a href="<?= base_url('admin/siswa'); ?>">Siswa</a></li>
...
<div class="container-fluid mt--6">
    <?= form_open_multipart('admin/edit_siswa/' . $siswa['id_siswa']) ?>
    <div class="row">
        <div class="col-md-8">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="sekolah" class="form-control" id="sekolah">
                                        ...
                                        <?php foreach ($sekolah as $row) : ?>
                                            <option value="<?= $row['id']; ?>" <?= ($siswa['sekolah'] === $row['id']) ? 'selected' : ''; ?>>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="guru" class="form-control" id="guru">
                                        ...
                                        <?php foreach ($akun_guru as $row) : ?>
                                            <option value="<?= $row['id']; ?>" <?=($siswa['guru'] === $row['id']) ?'selected':'';?>><?= strtoupper($row['nama']); ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                    <small class="text-danger"> <?= form_error('guru'); ?></small>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="col-md-4">
            <div class="card">
                ...
                <div class="card-footer">
                    <a href="<?= base_url('admin/siswa') ?>" class="btn btn-danger mr-4">Batal</a>
    ...
    <!-- script mencari data guru by sekolah -->
    <script>
        $(document).ready(function() {
            // Ketika tombol sekolah di klik
            $('#sekolah').on('change', function() {
                ...
                $.ajax({
                    url: "<?php echo base_url('admin/getGuru'); ?>",
                    type: "POST",
                    data: {
                        sekolah: sekolah
                    },
                    ...
                    success: function(data) {
                        // Tambahkan opsi guru ke dropdown
                        if (data.length > 0) {
                            // $('#guru').val(data.id);
                            // $('#guru').append('<option value="">Pilih..</option>');
                            $.each(data, function(key, value) {
                                $('#guru').append('<option value="' + value.id + '">' + value.nama + '</option>');
                            });
                        } else {
                            $('#guru').append('<option value="">Guru belum terdaftar disekolah ini</option>');
                        }
                    }
                });
            });
